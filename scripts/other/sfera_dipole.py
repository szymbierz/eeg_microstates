...
wspolrzedne = []
for s in fwd_fixed['src']:
    wspolrzedne.extend(s["rr"])
wspolrzedne = np.array(wspolrzedne)


procent = int(len(Q_srednia)*2/100)
top_indeksy = np.argsort(Q_srednia)[-procent:]
top_dipole = wspolrzedne[top_indeksy]
top_aktywnosc = Q_srednia[top_indeksy]
...
punkty_dipoli = ax.scatter(top_dipole[:,0], top_dipole[:,1], top_dipole[:,2], c=top_aktywnosc,
                           cmap="hot", s=5)
cbar = fig.colorbar(punkty_dipoli, ax=ax)
cbar.set_label("Aktywność (uV)")  
...
ax2.hist(top_aktywnosc,bins=10, color='blue', alpha=0.7)
ax2.set_title("Histogram średniej aktywności dipoli")
ax2.set_xlabel("Aktywność (uV)")
ax2.set_ylabel("Liczba dipoli")


np.save("top_dipole",top_dipole)
np.save("top_aktywnosc",top_aktywnosc)
plt.show()
plt.show()
plt.show()
